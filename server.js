...
// Public: get all active country currencies
app.get('/api/currencies', async (req, res) => {
  try {
    const { data, error } = await supabase
      ...
      .order('country');
    if (error) return sendSuccess(res, DEFAULT_CURRENCIES);
    return sendSuccess(res, data && data.length ? data : DEFAULT_CURRENCIES);
  } catch (e) {
    return sendSuccess(res, DEFAULT_CURRENCIES);
  }
});
...
app.get('/api/settings', async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return sendSuccess(res, { default_currency: 'SLL', currency_symbol: 'Le', currency_name: 'Sierra Leonean Leone', default_country: 'Sierra Leone', store_name: 'Zmafrdeal', use_geolocation: true });
    }
    return sendSuccess(res, data);
  } catch (e) {
