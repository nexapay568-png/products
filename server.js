...
// Hardcoded defaults (used if country_currencies table not yet created)
const DEFAULT_CURRENCIES = [
  ...
  { id:2, country:'Sierra Leone', flag:'🇸🇱', currency_code:'SLL', currency_symbol:'Le',   currency_name:'Sierra Leonean Leone', phone_code:'+232', ksh_rate:203.2,   is_main:true  },
  { id:3, country:'Ghana',        flag:'🇬🇭', currency_code:'GHS', currency_symbol:'GH₵',  currency_name:'Ghanaian Cedi',        phone_code:'+233', ksh_rate:0.0862,  is_main:true  },
  { id:4, country:'Zambia',       flag:'🇿🇲', currency_code:'ZMW', currency_symbol:'ZK',   currency_name:'Zambian Kwacha',       phone_code:'+260', ksh_rate:0.1394,  is_main:true  },
  { id:5, country:'Other',        flag:'🌍',  currency_code:'USD', currency_symbol:'$',    currency_name:'US Dollar',            phone_code:'',     ksh_rate:0.00774, is_main:false }
];
...
// Admin: delete a country/currency
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Determine currency_code from country (sent by client, or derive from defaults)
    const clientCurrency = req.body.currency_code;
    const countryMatch = DEFAULT_CURRENCIES.find(c => c.country === country);
    const resolvedCurrency = clientCurrency || (countryMatch ? countryMatch.currency_code : 'SLL');
...
app.get('/api/settings', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('site_settings')
      .select('default_currency, currency_symbol, currency_name, default_country, store_name, tagline, whatsapp_number, show_banners, show_flash_sale, maintenance_mode, pwa_enabled, use_geolocation')
      .eq('id', 1)
      .single();
    if (error) {
      // Return defaults if no settings exist yet
      return sendSuccess(res, { default_currency: 'SLL', currency_symbol: 'Le', currency_name: 'Sierra Leonean Leone', default_country: 'Sierra Leone', store_name: 'Zmafrdeal', use_geolocation: false });
    }
    return sendSuccess(res, data);
  } catch (e) {
    return sendSuccess(res, { default_currency: 'SLL', currency_symbol: 'Le', currency_name: 'Sierra Leonean Leone', default_country: 'Sierra Leone', store_name: 'Zmafrdeal', use_geolocation: false });
...
app.get('/api/locations/cities', async (req, res) => {
  try {
    ...
    let query = supabase.from('country_cities').select('id,city_name,sort_order').eq('active', true);
...
app.get('/api/payment-methods', async (req, res) => {
  try {
    const country = req.query.country || '';
    let query = supabase.from('country_payment_methods').select('id,name,description,icon_color,method_key,sort_order').eq('active', true);
    if (country) query = query.eq('country', country);
...
app.post('/api/admin/cities', requireAdmin, async (req, res) => {
  try {
    const { country, city_name, sort_order = 0, active = true } = req.body;
    ...
    if (error) return sendError(res, 500, error.message);
    return sendSuccess(res, data, 'City added');
  } catch (e) { return sendError(res, 500, e.message); }
});

app.put('/api/admin/cities/:id', requireAdmin, async (req, res) => {
  try {
    const { country, city_name, sort_order, active } = req.body;
    const updates = {};
    if (country !== undefined) updates.country = country;
...
app.post('/api/admin/payment-methods', requireAdmin, async (req, res) => {
  try {
    const { country, name, description = '', icon_color = '#888888', method_key, sort_order = 0, active = true } = req.body;
    if (!country || !name || !method_key) return sendError(res, 400, 'country, name, and method_key required');
    const { data, error } = await supabase.from('country_payment_methods').insert([{ country, name: name.trim(), description: description.trim(), icon_color, method_key: method_key.trim().toLowerCase().replace(/\s+/g,'_'), sort_order, active }]).select().single();
    if (error) return sendError(res, 500, error.message);
...
app.put('/api/admin/payment-methods/:id', requireAdmin, async (req, res) => {
  try {
    const { country, name, description, icon_color, method_key, sort_order, active } = req.body;
    const updates = {};
    if (country !== undefined) updates.country = country;
