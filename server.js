...
const SUPABASE_URL = 'https://hrokwllgudnpwbtunwjd.supabase.co';
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY;
const SUPABASE_ANON_KEY = 'sb_publishable_aN7cqMN77uzsVa9dVcvmZQ_4ZbqJOIc';
...
async function requireAdmin(req, res, next) {
  ...
  req.admin = data;
  req.user = { id: data.user_id || 'admin', role: 'admin' };
  next();
}

// Accepts either a valid user JWT or a valid admin key
...
app.post('/api/products', requireAuthOrAdmin, async (req, res) => {
  try {
    ...
    const product = {
      ...req.body,
      id: uuidv4(),
      seller_id: isAdminRequest ? (req.body.seller_id || req.user.id) : req.user.id,
