...
// ─── Supabase Config ───────────────────────────────────────────────────────────
const SUPABASE_URL = 'https://bqkixaympiqdrgenmtmp.supabase.co';
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY;
const SUPABASE_ANON_KEY = 'sb_publishable_8G_ojYIA3_pdLc52HVRXuA_fnSYJyad';
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Sign in immediately to get a valid session (signUp alone may not return one
    // if email confirmation is enabled or on some Supabase plan configurations)
    const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({ email, password });
    const session = signInData?.session || authData.session || null;
    const profile = { id: userId, email, full_name, country, phone: phone || null, role: 'buyer' };

    return sendSuccess(res, { user: authData.user, session, profile }, 'Account created successfully');
  } catch (e) {
...
app.get('/api/vouchers/public', async (req, res) => {
  try {
    const now = new Date().toISOString();
    let query = supabase.from('vouchers').select('id, code, discount_type, discount_value, min_order, usage_limit, usage_count, expires_at').eq('active', true).gt('usage_limit', supabase.raw('usage_count'));
    const { data, error } = await query.order('created_at', { ascending: false });
    if (error) return sendError(res, 500, error.message);
    const valid = (data || []).filter(v => !v.expires_at || new Date(v.expires_at) > new Date());
...
app.post('/api/admin/users/:id/ban', requireAuthOrAdmin, async (req, res) => {
  try {
    const { error } = await supabase.from('profiles').update({ role: 'buyer', banned: true }).eq('id', req.params.id);
    if (error) return sendError(res, 500, error.message);
