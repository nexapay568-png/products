...
app.get('/api/payment-methods', async (req, res) => {
  try {
    const country = req.query.country || '';
    let query = supabase.from('country_payment_methods').select('id,name,description,icon_color,method_key,sort_order,checkout_instructions').eq('active', true);
    if (country) query = query.eq('country', country);
...
app.post('/api/admin/payment-methods', requireAdmin, async (req, res) => {
  try {
    const { country, name, description = '', icon_color = '#888888', method_key, sort_order = 0, active = true, checkout_instructions = '' } = req.body;
    if (!country || !name || !method_key) return sendError(res, 400, 'country, name, and method_key required');
    const { data, error } = await supabase.from('country_payment_methods').insert([{ country, name: name.trim(), description: description.trim(), icon_color, method_key: method_key.trim().toLowerCase().replace(/\s+/g,'_'), sort_order, active, checkout_instructions: checkout_instructions.trim() }]).select().single();
    if (error) return sendError(res, 500, error.message);
...
app.put('/api/admin/payment-methods/:id', requireAdmin, async (req, res) => {
  try {
    const { country, name, description, icon_color, method_key, sort_order, active, checkout_instructions } = req.body;
    const updates = {};
    if (country !== undefined) updates.country = country;
